...
[English](README.md) | **Русский** | [Deutsch](README_DE.md) | [Français](README_FR.md) | [Nederlands](README_NL.md)
...
<sub>Та же карточка на других языках интерфейса: [English](media/demo_en.gif) · [Deutsch](media/demo_de.gif) · [Français](media/demo_fr.gif) · [Nederlands](media/demo_nl.gif)</sub>
...
- **Пять языков** — русский, английский, немецкий, французский и нидерландский из коробки. Язык берётся из профиля Home Assistant или задаётся явно: `language: ru | en | de | fr | nl`.
- **Визуальный редактор** — карточка отдаёт форму настроек, поэтому её можно собрать через интерфейс, не трогая YAML.
- **Без зависимостей** — один файл на чистом JS с Shadow DOM. Все entity-параметры, кроме `status_entity`, необязательны: блоки без сущности просто скрываются. Подстраивается под ширину — от колонки дашборда до узкого экрана телефона.
...
currency: "₾"
language: ru                                       # auto / ru / en / de / fr / nl (auto — язык из HA)
...
Названия и подписи переведены на все пять языков; параметр `name` перекрывает название.
