...
<sub>Демо с английским интерфейсом: [media/demo_en.gif](media/demo_en.gif) · [media/demo_en.mp4](media/demo_en.mp4)</sub>
...
- **Три языка** — русский, английский и немецкий из коробки. Язык берётся из профиля Home Assistant или задаётся явно: `language: ru | en | de`.
...
currency: "₾"
language: ru                                       # ru / en / de (по умолчанию — язык HA)
```
...
| `running_states` | нет | on, washing, стирка, run, spin, … | Состояния `status_entity`, считающиеся «работает». |
| `language` | нет | язык HA | `ru`, `en` или `de`. |
